...
public class ExerciseMapper {
    public static Exercise fromCreateExerciseDto(CreateExerciseDto dto) {
        ArrayList<BodyPart> bodyParts = new ArrayList<>(dto.getBodyParts().stream()
                .map((part) -> new BodyPart(null, part.getSlug(), part.getColor(), part.getIntensity(), part.getSide(),
                        null))
                .toList());
        return new Exercise(null, dto.getName(), dto.getDescription(), dto.getType(), bodyParts);
    }
    ...
    public static ExerciseDto fromExercise(Exercise exercise) {
        ArrayList<BodyPartDto> bodyParts = new ArrayList<>(exercise.getBodyParts().stream()
                .map((part) -> new BodyPartDto(part.getId(), part.getSlug(), part.getColor(), part.getIntensity(),
                        part.getSide()))
                .toList());
